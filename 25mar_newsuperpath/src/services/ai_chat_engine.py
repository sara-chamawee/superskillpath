...
class AIChatEngineService:
    ...
    def _try_llm_response(self, skill, session: ChatSession, step_type: StepType, user_message: str) -> Optional[str]:
        ...
        step_instructions = {
            StepType.INTRODUCTION: "แนะนำตัวเป็น Learning Companion อธิบายภาพรวมทักษะ สอบถามว่าพร้อมเริ่มไหม",
            StepType.BASELINE_ASSESSMENT: "ถามคำถามปลายเปิด 2-3 ข้อเพื่อประเมินความรู้เดิมของผู้เรียน",
            StepType.PERSONALIZED_PLAN: "นำเสนอแผนการเรียนรู้ที่ปรับตามผลประเมิน แสดงด้านที่จะเรียน",
            StepType.CONTENT_DELIVERY: f"อธิบายเนื้อหาด้าน '{current_area}' ยกตัวอย่างจริง ถามเช็คความเข้าใจ แนะนำคอร์สที่เกี่ยวข้อง",
            StepType.PRACTICE: f"ให้แบบฝึกหัด/สถานการณ์จำลองสำหรับด้าน '{current_area}' ให้ผู้เรียนตอบ",
            StepType.ASSESSMENT: f"ประเมินคำตอบผู้เรียน สรุปว่าผ่าน checklist ข้อไหนบ้าง แสดง progress",
            StepType.WRAP_UP: "สรุปทั้งหมดที่เรียน แสดงผล checklist สุดท้าย แนะนำทักษะต่อไป",
        }

        system_prompt = f"""คุณคือ Sorc AI — Learning Companion ที่ช่วยพัฒนาทักษะผ่านการสนทนาภาษาไทย

ทักษะที่กำลังเรียน: {s.name}
คำนิยาม: {s.definition[:500]}

ด้านที่กำลังเรียน (Area {area_idx + 1}/{len(s.assessment_criteria)}): {current_area}
...
ขั้นตอนปัจจุบัน: {step_type.name}
คำแนะนำ: {step_instructions.get(step_type, 'ตอบคำถามผู้เรียน')}

กฎ:
- ตอบเป็นภาษาไทย กระชับ เข้าใจง่าย
- ใช้ตัวอย่างจริงในชีวิตประจำวันหรือการทำงาน
- ถามคำถามเพื่อกระตุ้นการคิด
- อ้างอิง checklist items เมื่อเหมาะสม
- แนะนำคอร์สเรียนเมื่ออยู่ในขั้นตอน Content Delivery"""
